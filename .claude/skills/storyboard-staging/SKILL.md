...
description: 依畫面命題寫出每一格畫框裡實際有什麼——主體、動作、空間關係、道具。由 storyboard 呼叫，或使用者說畫面內容漏了東西、畫錯重點時使用。
disable-model-invocation: true
---

# storyboard-staging｜場面調度

**輸入**：`focus` ＋ `depiction_mode` ＋ `framing`（`source` 只當上下文）
...
## 起點是命題，不是 source ★最容易踩、也最致命

**不要讀 `source` 去挑可畫的名詞。讀 `focus`（畫面命題），把那一句話畫出來。**

`focus` 是 `storyboard-intent` 產出的一句完整中文句子。下筆前逐項拆它：

| 命題的成分 | 對應到畫面 |
|---|---|
| **主詞** | 畫面的**主體**。主詞是「你／他／人」→ **畫面必須有一個人** |
| **動詞** | 畫面的**動作或關係**。不能只把受詞畫出來就算 |
| **問號** | 畫面要留一個**明確的空位**（空容器、缺口、空白的思考泡泡） |
| **兩個對立子句** | **兩邊都要在畫面上**，只畫其中一邊等於沒畫 |

### 為什麼要立這條（2026-08-07 逐格稽核）

`storyboard-verify-quality` 有一條「具體可畫」準則。直接對著 `source` 執行它，
自然的做法就是**從整段話裡挑最顯眼、最好畫的名詞**——然後把那些名詞排進畫框。

技術上每個名詞都畫到了，但那是一份**清單**，不是一個**命題**。

| 格 | 命題 | 挑名詞畫成 | 少了什麼 |
|---|---|---|---|
| s07 | 錢**換掉**了乾等的時間 | 計程車、掃把、排隊柱三個 icon 並排 | 「換」這個動作 |
| s02 | **你的**一年生命被標價 49 萬 | 一疊鈔票 vs 一根蒼白短條 | 那個「你」 |
| s11 | 閒晃半天是實實在在**虧掉**五塊 | 敲鎚的人 vs 躺著的人 | 那筆帳 |
| s08 | 一年**要多少錢？** | 灰條右端接一段橘色 | 那個問號 |
| s23 | **以為**以後會閒，**其實**更忙 | 三個填充遞增的面板 | 「以為」那一邊 |
| s24 | 你**想像不出**兩鐘頭值多少錢 | 兩隻手掌，一手金幣一手流沙 | 那個想不出來的人 |

**前 24 格裡，12 格「命題＝字面名詞」的全部通過，12 格「命題≠字面名詞」的全部畫錯重點。**

### 怎麼畫「未知」（全域禁止文字之下）

問號是字元，`negative` 的 `text, lettering` 會擋，寫進 prompt 也多半畫成亂碼。
命題是問句時，用**形狀**表示未知：

| ✓ 可行 | 為什麼 |
|---|---|
| 一個只有輪廓、裡面完全空的容器／托盤 | 「該放什麼進去？」 |
| 桌面上一個和實體同大小的**凹槽**，裡面什麼都沒有 | 「這裡少了一塊」 |
| 一個大的、完全空白的思考泡泡輪廓 | 「他想不出來」 |
| 把問號當成**雕塑實體**描述形狀（`a large solid curved hook shape with a separate round dot below it`） | 繞開文字禁令，但仍有風險，只在非它不可時用 |

空位要**明講「裡面什麼都沒有」**（`with absolutely nothing inside it`）——
不講的話模型會自己填滿。

### ★命題主詞是人 → 那個人一定要寫服裝（2026-08-07 實測）

P2 說「主詞是人就要有人」，但只寫 `a person` **不夠**。
模型會畫出一個沒穿衣服、通體單色的人形。

實測：s24 的 B 版寫 `a person seen from the chest up ... with one hand resting on the chin`
→ 命題（想像不出來）完全成立，但那個人畫成**通體紅色、看起來沒穿衣服**。
...
> ⚠ **`workflow = text_with_character` 不等於「這一格有觀察者」**（2026-08-07）。
> 現在只要畫面上有**任何人**就會掛參考圖（見 `storyboard-depiction`），
...
> （實測見 `storyboard-depiction`）。**不要寫「這不是觀察者」「沒有圓框眼鏡」這類排除句
> 去對抗它——實測完全無效。** 路人跟觀察者的區別靠服裝與動作，不靠臉。
>
