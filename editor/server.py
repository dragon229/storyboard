...
7. staging：英文，**只寫畫框內容**（主體、動作、空間關係、道具的物理狀態）。
   ...
   ・有觀察者時背景必須寫具體（場所＋兩個具體元素＋色調方向），
     否則參考圖的米白素底會滲進來讓整張褪色。
8. workflow：畫面上**有任何人**就填 "text_with_character"——觀察者、路人、群眾、
   只有一隻手都算；完全沒有人才填 "text_only"。跟呈現手法無關。
   （純文字路徑畫出來的人跟參考圖路徑不同調，有人的格全部走參考圖才不會分成兩種畫法。）
   ★只有路人、觀察者沒出現的格，staging 不寫觀察者的外觀或服裝，照一般人物寫
     「描述詞＋具體衣物＋顏色」即可。掛了參考圖的格路人的臉本來就會像觀察者，
     這是已接受的代價，**不要寫「這不是觀察者」「沒有圓框眼鏡」這類排除句，實測無效。**
